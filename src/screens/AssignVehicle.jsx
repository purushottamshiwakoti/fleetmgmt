...
const AssignVehicle = () => {
  ...
  const handleAssign = async () => {
    try {
      const response = await axios.post(
        ...
        { vehicleId: tripId, deiverId: driverId }
      );
  ...
  return (
    <ScrollView>
      <Header title={"Assign Vehicle"} />
      <Text>Assign Vehicle</Text>
      <View>
      ...
      <View>
        <Text style={{ marginLeft: 50 }}>Assign Vehicle </Text>
        <SelectDropdown
          data={trip.map((item) => item.vehicleModel)}
