...
const ViewVehicles = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `https://fleetmgmt.vercel.app/api/vehicle/${id}`
      );
  ...
  return (
    <PaperProvider>
      <SafeAreaView>
        <Header title={"View Vehicle"} />
        <View>
          <DataTable>
            <DataTable.Header>
              <DataTable.Title>Plate Number</DataTable.Title>
              <DataTable.Title numeric>Vehicle Model</DataTable.Title>
            ...
            {vehicles.length ? (
              vehicles.map((item) => (
                <DataTable.Row key={item.id}>
                  ...
                  <DataTable.Cell numeric>{item.vehicleModel}</DataTable.Cell>
